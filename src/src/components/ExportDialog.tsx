...
export function ExportDialog({ client, snapshot, onNotice, onClose }: { client: EditorClient; snapshot: ProjectSnapshot; onNotice: (notice: string) => void; onClose: () => void }) {
  ...
  const applyStatus = useCallback((data: Record<string, unknown>) => {
    ...
    if (job.state === "completed" && destination) {
      ...
      setRunning(true);
      setPhase(job.state === "queued" ? "Waiting for export worker…" : "Rendering immutable revision…");
    }
  }, []);
  ...
  return <div className="export-dialog"><div className="settings-header"><div><p className="eyebrow">Deliver</p><h2>Export video</h2><p>Export captures revision {snapshot.document.revision} and will not change if you continue editing.</p></div><Button variant="ghost" size="icon" aria-label="Close export dialog" onClick={onClose}><X aria-hidden="true" /></Button></div><div className="export-preview-card"><div className="export-icon"><Film aria-hidden="true" /></div><div><strong>{snapshot.document.name}</strong><span>{outputWidth} × {outputHeight} · H.264 / AAC · {snapshot.document.profile.fpsNum / snapshot.document.profile.fpsDen} fps</span></div><span className="export-revision">r{snapshot.document.revision}</span></div><div className="export-options"><label className="field-group"><span className="field-label">Resolution</span><select value={resolution} onChange={(event) => setResolution(Number(event.target.value) as 720 | 1080)} disabled={running}><option value={1080}>1080p · {widthFor(1080)} × {heightFor(1080)}</option><option value={720}>720p · {widthFor(720)} × {heightFor(720)}</option></select></label><label className="toggle-row"><span>SRT caption sidecar</span><input type="checkbox" checked={srt} onChange={(event) => setSrt(event.target.checked)} disabled={running} /><small>Burn-in remains available from the captioned timeline.</small></label></div><div className="export-progress" aria-live="polite"><div className="export-progress-header"><span>{phase}</span><span>{Math.round(progress * 100)}%</span></div><div className="progress-track"><span style={{ width: `${Math.round(progress * 100)}%` }} /></div>{jobId ? <small>Job {jobId}</small> : null}</div><div className="dialog-actions">{complete && outputPath ? <><Button variant="secondary" onClick={() => void showOutput("play")}><Play aria-hidden="true" />Play</Button><Button variant="secondary" onClick={() => void showOutput("show_file")}><ExternalLink aria-hidden="true" />Show file</Button></> : null}{running ? <Button variant="ghost" onClick={() => void cancel()}><Square aria-hidden="true" />Cancel</Button> : <Button onClick={() => void start()} disabled={complete}><Download aria-hidden="true" />{complete ? "Exported" : "Export"}</Button>}{complete ? <span className="export-success"><CheckCircle2 aria-hidden="true" />Finalized safely</span> : null}</div></div>;
}
